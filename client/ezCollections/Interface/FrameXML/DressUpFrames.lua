TRANSMOGRIFY_TOOLTIP_APPEARANCE_KNOWN = "已收集"
local ADDON_NAME = ...;
...
local function SetupDressUpModel(model)
	...
	-- 修复：portraitZoomMultiplier 可能为 nil(魔改种族查表失败)，兜底防除nil崩溃
...
function DressUpOutfitDetailsSlotMixin:OnEnter()
	...
	--elseif self.slotState == OUTFIT_SLOT_STATE_UNCOLLECTED then
		...
		if C_TransmogCollection.PlayerKnowsSource(self.transmogID) then
			...
			self.tooltipSourceIndex, self.tooltipCycle = CollectionWardrobeUtil.SetAppearanceTooltip(GameTooltip, sources, self.transmogID, self.tooltipSourceIndex, showUseError, inLegionArtifactCategory, subheaderString, false, self.item and LE_TRANSMOG_TYPE_APPEARANCE or LE_TRANSMOG_TYPE_ILLUSION);
		else
			GameTooltip:AddLine(self.name, GetItemQualityColor(select(3, GetItemInfo(self.item))));
			local slotName = TransmogUtil.GetSlotName(self.slotID);
			GameTooltip_AddColoredLine(GameTooltip, _G[slotName], HIGHLIGHT_FONT_COLOR);
	...
	self:GetParent():EnableKeyboard(self.tooltipCycle and ezCollections.Config.Wardrobe.TooltipCycleKeyboard);
	if self then
		self:EnableMouseWheel(ezCollections.Config.Wardrobe.TooltipCycleMouseWheel);
	end
end
...
function DressUpOutfitDetailsSlotMixin:OnLeave()
	...
	self:GetParent():EnableKeyboard(false);
