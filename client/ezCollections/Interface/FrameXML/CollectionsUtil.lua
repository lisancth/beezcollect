...
function CollectionWardrobeUtil.SetAppearanceTooltip(tooltip, sources, primarySourceID, selectedIndex, showUseError, inLegionArtifactCategory, subheaderString, showSets, transmogType)
	...
	if transmogType == LE_TRANSMOG_TYPE_ILLUSION then
		local visualID;
		visualID, name = C_TransmogCollection.GetIllusionSourceInfo(headerSourceID);
		nameColor = NORMAL_FONT_COLOR;
	else
		name, nameColor = CollectionWardrobeUtil.GetAppearanceNameTextAndColor(sources[headerIndex], inLegionArtifactCategory);
	end
		...
		elseif info.RaceMask == hordeRaceMask then
			...
			for i, name in ipairs(ezCollections.RaceIDToName) do
				if bit.band(info.RaceMask, bit.lshift(1, i - 1)) ~= 0 then
					-- 修复：魔改种族本地化缺失返回nil导致拼接崩溃，用种族代码兜底
					races = races .. (races ~= "" and ", " or "") .. (ezCollections.L["Race."..i] or name or tostring(i));
	...
	if info and info.ClassMask then
		...
		for i, name in ipairs(ezCollections.ClassIDToName) do
			if bit.band(info.ClassMask, bit.lshift(1, i - 1)) ~= 0 then
				-- 修复：魔改职业(MONK武僧/DEMONHUNTER恶魔猎人)在3.3.5原版职业名表缺失返回nil导致拼接崩溃，用职业代码兜底
				classes = classes .. (classes ~= "" and ", " or "") .. (LOCALIZED_CLASS_NAMES_MALE[name] or name);
	...
	if transmogType == LE_TRANSMOG_TYPE_ILLUSION then
		local enchant = ezCollections:GetEnchantFromScroll(headerSourceID);
		if enchant then
			local variants = ezCollections:GetScrollVariantsFromEnchant(enchant);
			if variants then
				for _, variant in ipairs(variants) do
					if variant ~= headerSourceID then
						local _, name = C_TransmogCollection.GetIllusionSourceInfo(variant);
						GameTooltip_AddColoredLine(tooltip, format(ezCollections.L["Tooltip.Transmog.EnchantVariantFormat"], name), GRAY_FONT_COLOR, true);
					end
				end
			end
		end
	...
	if ezCollections.Developer and transmogType == LE_TRANSMOG_TYPE_ILLUSION then
		GameTooltip_AddColoredLine(tooltip, "ID "..headerSourceID.."  ||  "..(ezCollections:GetEnchantFromScroll(headerSourceID) or ezCollections:GetHiddenEnchant()), GRAY_FONT_COLOR, true);
