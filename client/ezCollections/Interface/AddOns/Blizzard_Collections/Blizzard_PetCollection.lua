...
function PetJournal_UpdatePetList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedPets and showPets ) then
			...
			local petID, _, isCollected, _, _, isFavorite, _, creatureName, icon, petType, summonSpellID = C_PetJournal.GetPetInfoByIndex(index);
			-- 修复：tooltip 用的 spellID 原来是 petID(speciesID 如10673)，会撞上无关法术(显示旋风斩等)。
			-- 改用真正的召唤法术ID(GetPetInfoByIndex 第11返回值=creatureID，如灰猫的7385)。
			local spellID = summonSpellID or petID;
			...
			button.AccountOverlay:SetShown(ezCollections:IsActivePetAccountPet(petID));
...
function PetJournalSummonButton_UpdateTooltip(self)
	-- 修复：原 SetHyperlink("spell:"..spellID) 用宠物ID当法术，会显示无关技能(攻击/旋风斩)。
	-- 改为直接显示宠物名+介绍(从宠物数据取)，不再依赖法术链接。
	local petID = self.index and C_PetJournal.GetPetInfoByIndex(self.index) or self.spellID;
	local name, _, _, _, sourceText, description = C_PetJournal.GetPetInfoBySpeciesID(petID);
...
function PetJournal_UpdatePetDisplay()
	if ( PetJournal.selectedPetID ) then
		...
		if creatureName and ( PetJournal.PetDisplay.lastDisplayed ~= spellID ) then
			...
			PetJournal.PetDisplay.InfoButton.SubscriptionInfo:SetText(ezCollections:IsActivePetAccountPet(PetJournal.selectedPetID) and ezCollections.L["Pet.Account.Details.Info"] or ezCollections:IsActivePetSubscriptionPet(PetJournal.selectedPetID) and ezCollections.L["Pet.Subscription.Details.Info"] or "");
			...
			if creatureDisplayID == 0 then
				...
				-- 修复：DressUpModel 默认显示玩家/残留旧模型，直接 SetCreature 不切换(卡在上一个宠物)。
				-- 先 ClearModel 清掉旧模型，再设新生物，3D 预览才会跟着选中的宠物切换。
				PetJournal.PetDisplay.ModelFrame:ClearModel();
...
function PetJournal_Select(index)
	-- 修复：selectedSpellID 改用真召唤法术ID(第11返回值)，与列表项 button.spellID 保持一致，
	-- 否则选中高亮比较(selectedSpellID == button.spellID)失效，且 tooltip 撞无关法术。
	local petID, _, _, _, _, _, _, _, _, _, summonSpellID = C_PetJournal.GetPetInfoByIndex(index);
	local spellID = summonSpellID or petID;
...
function PetJournalFilterDropDown_Initialize(self, level)
	...
	if level == 1 then
		...
		if ezCollections:IsActivePetAccount() then
			info.text = ezCollections.L["Pet.Filter.Account"];
			info.func = function(_, _, _, value)
							C_PetJournal.SetFilterChecked(LE_PET_JOURNAL_FILTER_ACCOUNT, value);
						end
			info.checked = C_PetJournal.IsFilterChecked(LE_PET_JOURNAL_FILTER_ACCOUNT);
			info.isNotRadio = true;
			UIDropDownMenu_AddButton(info, level);
		end
