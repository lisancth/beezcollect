...
function MountJournal_UpdateMountList()
	...
	for i=1, #buttons do
		...
		if ( displayIndex <= numDisplayedMounts and showMounts ) then
			...
			button.AccountOverlay:SetShown(ezCollections:IsActiveMountAccountMount(mountID));
...
function MountJournal_UpdateMountDisplay()
	if ( MountJournal.selectedMountID ) then
		...
		if creatureName and ( MountJournal.MountDisplay.lastDisplayed ~= spellID ) then
			...
			MountJournal.MountDisplay.InfoButton.SubscriptionInfo:SetText(ezCollections:IsActiveMountAccountMount(MountJournal.selectedMountID) and ezCollections.L["Mount.Account.Details.Info"] or ezCollections:IsActiveMountSubscriptionMount(MountJournal.selectedMountID) and ezCollections.L["Mount.Subscription.Details.Info"] or "");
...
function MountJournalFilterDropDown_Initialize(self, level)
	...
	if level == 1 then
		...
		if ezCollections:IsActiveMountAccount() then
			info.text = ezCollections.L["Mount.Filter.Account"];
			info.func = function(_, _, _, value)
							C_MountJournal.SetCollectedFilterSetting(LE_MOUNT_JOURNAL_FILTER_ACCOUNT, value);
						end
			info.checked = C_MountJournal.GetCollectedFilterSetting(LE_MOUNT_JOURNAL_FILTER_ACCOUNT);
			info.isNotRadio = true;
			UIDropDownMenu_AddButton(info, level);
		end
