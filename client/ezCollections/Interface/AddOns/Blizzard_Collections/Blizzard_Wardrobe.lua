...
function WardrobeFrame_OnLoad(self)
	...
	WardrobeFrameTab4:SetText(ezCollections.L["Library"]);
...
function WardrobeCollectionFrame_SetTab(tabID)
	...
	if ( tabID == TAB_ITEMS ) then
		...
		overlayLayer = WardrobeCollectionFrame.activeFrame.OverlayLayer;
	elseif ( tabID == TAB_SETS ) then
		...
		if ( atTransmogrifier )  then
			...
			overlayLayer = WardrobeCollectionFrame.activeFrame.RightInset.OverlayLayer;
		end
		WardrobeCollectionFrame.searchBox:SetEnabled(true);
	...
	CollectionsClickCollect:Attach(WardrobeCollectionFrame:GetParent(), overlayLayer, ezCollections.L["ClickCollect.Wardrobe.Header"], ezCollections.L["ClickCollect.Wardrobe.Text"]);
end
...
function WardrobeItemsCollectionMixin:UpdateWeaponDropDown()
	...
	if ( not isWeapon ) then
		...
		UIDropDownMenu_Initialize(self, WardrobeCollectionFrameWeaponDropDown_Init); -- Erase all buttons, otherwise the menu will keep ballooning from every manual call to WardrobeCollectionFrameWeaponDropDown_Init
...
function WardrobeItemsCollectionMixin:FilterVisuals()
	...
	local showBattlePass = ezCollections:GetCVarBool("transmogrifyShowBattlePass");
	...
	for i = 1, #visualsList do
		if ( isAtTransmogrifier ) then
			...
			elseif not visualsList[i].isCollected and showClaimable and visualsList[i].isClaimable then
				tinsert(filteredVisualsList, visualsList[i]);
			elseif not visualsList[i].isCollected and showPurchasable and (visualsList[i].isStoreSource or visualsList[i].isSubscriptionSource) then
				tinsert(filteredVisualsList, visualsList[i]);
			elseif not visualsList[i].isCollected and showBattlePass and visualsList[i].isBattlePassSource then
				tinsert(filteredVisualsList, visualsList[i]);
...
function WardrobeItemsCollectionMixin:SortVisuals()
	local sortFavorites = C_TransmogCollection.GetSortFavorites();
	local sortCollected = C_TransmogCollection.GetSortCollected();
	local comparison = function(source1, source2)
		if sortCollected and ( source1.isCollected ~= source2.isCollected ) then
			return source1.isCollected;
		end
		if sortCollected and ( source1.isUsable ~= source2.isUsable ) then
			return source1.isUsable;
		end
		if sortFavorites and ( source1.isFavorite ~= source2.isFavorite ) then
			return source1.isFavorite;
		end
		if ( source1.isHideVisual ~= source2.isHideVisual ) then
			return source1.isHideVisual;
		end
		if sortCollected and ( source1.hasActiveRequiredHoliday ~= source2.hasActiveRequiredHoliday ) then
...
function WardrobeItemsCollectionMixin:UpdateItems()
	...
	if ( self.transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
		...
		appearanceVisualID = appearanceSourceID; -- ezCollections: force using sourceID here, as visualID might pointing to a weapon from another hand
	...
	for i = 1, self.PAGE_SIZE do
		...
		if ( visualInfo ) then
			...
			if self.transmogType == LE_TRANSMOG_TYPE_ILLUSION then
				...
				model:SetModelScale(isArmor and 5 or 1);
			...
			if needsDressing then
				model:Undress();
				model.previewedSource = self:GetAnAppearanceSourceFromVisual(visualInfo.visualID);
				if ( isArmor ) then
					if hasItemData then
						model:TryOn(model.previewedSource);
					end
				elseif ( appearanceVisualID ) then
					-- appearanceVisualID is only set when looking at enchants
					model:SetItemAppearance(appearanceVisualID, model.previewedSource);
				else
					model:SetItemAppearance(model.previewedSource);
				end
			end
			...
			model.ClaimQuest:SetShown(not visualInfo.isCollected and visualInfo.isClaimable);
			model.BattlePassButton:SetShown(not model.ClaimQuest:IsShown() and not visualInfo.isCollected and visualInfo.isBattlePassSource);
			model.StoreButton:SetShown(not model.ClaimQuest:IsShown() and not model.BattlePassButton:IsShown() and not visualInfo.isCollected and visualInfo.isStoreSource and not visualInfo.isSubscriptionSource);
			model.SubscriptionButton:SetShown(not model.ClaimQuest:IsShown() and not model.BattlePassButton:IsShown() and not visualInfo.isCollected and not visualInfo.isStoreSource and visualInfo.isSubscriptionSource);
			model.StoreSubscriptionButton:SetShown(not model.ClaimQuest:IsShown() and not model.BattlePassButton:IsShown() and not visualInfo.isCollected and visualInfo.isStoreSource and visualInfo.isSubscriptionSource);
			model.BattlePassUnderlay:SetShown(model.BattlePassButton:IsShown());
			...
			model.SolidBackground:SetShown(not model.BattlePassUnderlay:IsShown() and not model.StoreUnderlay:IsShown() and not model.SubscriptionUnderlay:IsShown());
	...
	if ( pendingTransmogModelFrame ) then
		...
		pendingTransmogModelFrame.BattlePassButton:SetFrameLevel(self.PendingTransmogFrame:GetFrameLevel() + 1);
...
function WardrobeItemsCollectionMixin:UpdateProgressBar()
	...
	collected = C_TransmogCollection.GetCategoryCollectedCount(self.activeCategory, C_TransmogCollection.GetGroup());
	total = C_TransmogCollection.GetCategoryTotal(self.activeCategory, C_TransmogCollection.GetGroup());
	WardrobeCollectionFrame_UpdateProgressBar(collected, total);
end

function WardrobeCollectionFrame_UpdateProgressBar(value, max)
	-- 修复：value/max 可能为 nil；HEIRLOOMS_PROGRESS_FORMAT 仅在 ruRU 本地化定义，中/英文客户端为 nil 导致 SetFormattedText 崩溃
	value = value or 0;
	max = max or 0;
	WardrobeCollectionFrame.progressBar:SetMinMaxValues(0, max);
	WardrobeCollectionFrame.progressBar:SetValue(value);
	WardrobeCollectionFrame.progressBar.text:SetFormattedText(HEIRLOOMS_PROGRESS_FORMAT or "%d/%d", value, max);
end
...
function WardrobeItemsCollectionMixin:SelectVisual(visualID)
	...
	local sourceID = self:GetAnAppearanceSourceFromVisual(visualID, true);
...
function WardrobeCollectionFrame_OpenTransmogLink(link, transmogType)
	...
	if ( linkType == "transmogillusion" or linkType == "transmogappearance" ) then
		local sourceID = tonumber(id);
		WardrobeCollectionFrame_SetTab(TAB_ITEMS);
		WardrobeCollectionFrame.ItemsCollectionFrame:GoToSourceID(sourceID, nil, linkType == "transmogillusion" and LE_TRANSMOG_TYPE_ILLUSION or LE_TRANSMOG_TYPE_APPEARANCE);
...
function WardrobeItemsCollectionMixin:SetAppearanceTooltip(frame)
	...
	self.tooltipItemFrame = C_TransmogCollection.GetGroup() and frame;
	self:RefreshAppearanceTooltip();
end

function WardrobeItemsCollectionMixin:RefreshAppearanceTooltip(cycle)
	...
	local selectedSource = WardrobeCollectionFrame_SetAppearanceTooltip(self, sources, chosenSourceID, ezCollections.Config.Wardrobe.ShowSetsInAppearances);
	if cycle and selectedSource then
		self.tooltipItemFrame.previewedSource = selectedSource.sourceID;
		if ezCollections.Developer then
			self.tooltipItemFrame:TryOn(self.tooltipItemFrame.previewedSource);
		end
	end
end
...
function WardrobeItemsModelMixin:OnMouseDown(button)
	...
	if ( IsModifiedClick("CHATLINK") ) then
		...
		if ( transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
			local name;
			name, link = C_TransmogCollection.GetIllusionStrings(self.previewedSource or self.visualInfo.sourceID);
		elseif self.previewedSource then
			link = select(6, C_TransmogCollection.GetAppearanceSourceInfo(self.previewedSource));
		else
	...
	elseif ( IsModifiedClick("DRESSUP") ) then
		...
		if ( transmogType == LE_TRANSMOG_TYPE_APPEARANCE ) then
			local sourceID = self.previewedSource or self:GetParent():GetAnAppearanceSourceFromVisual(self.visualInfo.visualID);
			-- don't specify a slot for ranged weapons
		...
		elseif ( transmogType == LE_TRANSMOG_TYPE_ILLUSION ) then
			...
			local sourceID = self.previewedSource or self:GetParent():GetAnAppearanceSourceFromVisual(self.visualInfo.visualID);
			local enchant = ezCollections:GetEnchantFromScroll(sourceID);
	...
	-- 修复：visualInfo 尚未加载完成(nil)时点击会崩，直接忽略本次点击
	if not self.visualInfo then return end
...
function WardrobeCollectionFrame_SetAppearanceTooltip(contentFrame, sources, primarySourceID, showSets)
	...
	if ( not selectedIndex ) then
		if contentFrame and contentFrame.tooltipItemFrame then
			for i = 1, #sources do
				if sources[i].sourceID == contentFrame.tooltipItemFrame.previewedSource and not sources[i].useError then
					selectedIndex = i;
					break;
				end
			end
	...
	self.tooltipSourceIndex, self.tooltipCycle = CollectionWardrobeUtil.SetAppearanceTooltip(GameTooltip, sources, primarySourceID, selectedIndex, showUseError, inLegionArtifactCategory, nil, showSets, contentFrame.transmogType);
...
function WardrobeCollectionFrameRightClickDropDown_Init(self, level, menuList)
	...
	local sets = skin and skin.Sets;
	if C_TransmogCollection.GetGroup() and not ezCollections:IsSingleSourceVisual(appearanceID) then
		sets = { };
		for _, sourceID in ezCollections:GetVisualSources(appearanceID) do
			local info = ezCollections:GetSkinInfo(sourceID);
			if info and info.Sets then
				for _, setID in ipairs(info.Sets) do
					if not tContains(sets, setID) then
						table.insert(sets, setID);
					end
				end
			end
		end
	...
	local devID, devSkin, devSource;
	if ezCollections.Developer then
		devID, devSkin = appearanceID, skin;
		if self.activeFrame.previewedSource then
			devID = self.activeFrame.previewedSource;
			devSkin = ezCollections:GetSkinInfo(devID);
		end
		for _, source in ipairs(C_TransmogCollection.GetAppearanceSources(appearanceID)) do
			if source.sourceID == devID then
				devSource = source;
				break;
			end
		end
	...
	if level ~= 1 then
		...
		elseif menuList == "SourceMask" then
			appearanceID, skin = devID, devSkin;
	...
	for i = 1, #sources do
		if ( sources[i].isCollected and not sources[i].useError ) or ezCollections.Developer then
			...
			elseif sources[i].useError then
				info.colorCode = RED_FONT_COLOR_CODE;
			end
			UIDropDownMenu_AddButton(info);
		end
...
function WardrobeFilterDropDown_InitializeItems(self, level)
	...
	if level == 1 then
		...
		info.text = ezCollections.L["Transmog.Sets.Filter.Group"]
		info.func = function(_, _, _, value)
						C_TransmogCollection.SetGroup(value);
					end
		info.checked = C_TransmogCollection.GetGroup();
		info.isNotRadio = true;
		UIDropDownMenu_AddButton(info, level)
		...
		if atTransmogrifier then
			...
			info.text = ezCollections.L["Transmog.Items.Filter.BattlePass"];
			info.func = function(_, _, _, value)
				ezCollections:SetCVarBool("transmogrifyShowBattlePass", value);
				ezCollections:RaiseEvent("TRANSMOG_COLLECTION_UPDATED");
			end;
			info.checked = function() return ezCollections:GetCVarBool("transmogrifyShowBattlePass"); end;
			UIDropDownMenu_AddButton(info, level);
		end
	else
		...
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 6 then
			...
			local items =
			{
				{  0, ezCollections.L["Transmog.Items.Filter.Sort.Visual"] },
				{ -1, ezCollections.L["Transmog.Items.Filter.Sort.Name"] },
				{  1, ezCollections.L["Transmog.Items.Filter.Sort.Name.Invert"] },
				{ -2, ezCollections.L["Transmog.Items.Filter.Sort.ID"] },
				{  2, ezCollections.L["Transmog.Items.Filter.Sort.ID.Invert"] },
			};

			for _, item in ipairs(items) do
				local value;
				value, info.text = unpack(item);
				info.func = function(_, _, _, _) C_TransmogCollection.SetSort(value); UIDropDownMenu_Refresh2(WardrobeFilterDropDown, 6, level); end;
				info.checked = function() return C_TransmogCollection.GetSort() == value end;
				UIDropDownMenu_AddButton(info, level);
			end
			...
			info.text = ezCollections.L["Transmog.Items.Filter.SortFavorites"]
			info.func = function(_, _, _, value) C_TransmogCollection.SetSortFavorites(value); end
			info.checked = function() return C_TransmogCollection.GetSortFavorites() end;
			UIDropDownMenu_AddButton(info, level)

			info.text = ezCollections.L["Transmog.Items.Filter.SortCollected"]
			info.func = function(_, _, _, value) C_TransmogCollection.SetSortCollected(value); end
			info.checked = function() return C_TransmogCollection.GetSortCollected() end;
			UIDropDownMenu_AddButton(info, level)
		elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == 1 then
			...
			for i = 1, numSources do
				...
				if i == TRANSMOG_SOURCE_BOSS_DROP then
					info.hasArrow = true;
					info.value = 1;
				else
					info.hasArrow = false;
		...
		elseif level == 3 and UIDROPDOWNMENU_MENU_VALUE == 1 then
			info.hasArrow = true;
			for i = 1, 3 do
				local id = ezCollections.L["InstanceTier."..i];
				info.text = id;
				info.value = id;
				info.func = function(_, _, _, value)
					C_TransmogCollection.SetBossFilterByInstanceTier(id, value);
					UIDropDownMenu_Refresh2(WardrobeFilterDropDown, id, 4);
				end
				info.checked = function() return not C_TransmogCollection.IsBossFilterCheckedByInstanceTier(id) end;
				UIDropDownMenu_AddButton(info, level);
			end
		elseif level == 4 then
			info.hasArrow = true;
			for type = INSTANCE_TYPE_DUNGEON, INSTANCE_TYPE_RAID do
				for id in ezCollections:Ordered(ezCollections.Instances) do
					local instance = ezCollections:GetInstanceInfo(id);
					if instance and instance.Tier == UIDROPDOWNMENU_MENU_VALUE and instance.Type == type then
						for encounterID in pairs(ezCollections.Encounters) do
							local encounter = ezCollections:GetEncounterInfo(encounterID);
							if encounter and encounter.Map == id then
								local prefix;
								if instance.Type == INSTANCE_TYPE_DUNGEON then
									prefix = [[|TInterface\AddOns\ezCollections\Interface\Minimap\Dungeon:22:22:0:0:32:32:5:27:5:27|t]];
								elseif instance.Type == INSTANCE_TYPE_RAID then
									prefix = [[|TInterface\AddOns\ezCollections\Interface\Minimap\Raid:22:22:0:0:32:32:5:27:5:27|t]];
								end
								if prefix then
									info.text = format("%s %s", prefix, instance.Name);
									info.value = id;
									info.func = function(_, _, _, value)
										C_TransmogCollection.SetBossFilterByInstance(id, value);
										UIDropDownMenu_Refresh2(WardrobeFilterDropDown, 1, 3);
										UIDropDownMenu_Refresh2(WardrobeFilterDropDown, id, 5);
									end
									info.checked = function() return not C_TransmogCollection.IsBossFilterCheckedByInstance(id) end;
									UIDropDownMenu_AddButton(info, level);
								end
								break
							end
		...
		elseif level == 5 then
			local difficulties = { { }, { }, { }, { }, { }, { } };
			for id in ezCollections:Ordered(ezCollections.Encounters) do
				local encounter = ezCollections:GetEncounterInfo(id);
				if encounter and encounter.Map == UIDROPDOWNMENU_MENU_VALUE then
					table.insert(difficulties[encounter.DifficultyIndex], id);
				end
			end
			local numDifficulties = 0;
			for index, encounters in ipairs(difficulties) do
			...
			for index, encounters in ipairs(difficulties) do
				for _, id in ipairs(encounters) do
					local encounter = ezCollections:GetEncounterInfo(id);
					if numDifficulties > 1 then
						if lastDifficulty ~= encounter.DifficultyIndex then
							if lastDifficulty then
								UIDropDownMenu_AddSeparator(level);
							end
							info.colorCode = NORMAL_FONT_COLOR_CODE;
							info.text = encounter.Difficulty;
							info.func = function(_, _, _, value)
								C_TransmogCollection.SetBossFilterByInstanceDifficulty(encounter.Map, encounter.DifficultyIndex, value);
								UIDropDownMenu_Refresh2(WardrobeFilterDropDown, 1, 3);
								UIDropDownMenu_Refresh2(WardrobeFilterDropDown, ezCollections:GetInstanceInfo(encounter.Map).Tier, 4);
								UIDropDownMenu_Refresh2(WardrobeFilterDropDown, encounter.Map, 5);
							end
							info.checked = function() return not C_TransmogCollection.IsBossFilterCheckedByInstanceDifficulty(encounter.Map, encounter.DifficultyIndex) end;
							UIDropDownMenu_AddButton(info, level);
							info.colorCode = nil;
						end
						lastDifficulty = index;
					end
					info.text = encounter.Name;
					info.func = function(_, _, _, value)
						C_TransmogCollection.SetBossFilter(id, value);
						UIDropDownMenu_Refresh2(WardrobeFilterDropDown, 1, 3);
						UIDropDownMenu_Refresh2(WardrobeFilterDropDown, ezCollections:GetInstanceInfo(encounter.Map).Tier, 4);
						UIDropDownMenu_Refresh2(WardrobeFilterDropDown, encounter.Map, 5);
					end
					info.checked = function() return not C_TransmogCollection.IsBossFilterChecked(id) end;
					UIDropDownMenu_AddButton(info, level);
				end
			end
		end
...
function WardrobeSetsCollectionMixin:DisplaySet(setID)
	...
	for i = 1, #sortedSources do
		...
		local battlePassSource = ezCollections:GetBattlePassSetSource(setID, slot);
		itemFrame.ClaimQuest:SetShown(not sortedSources[i].collected and ezCollections:CanClaimSetSlotSkin(setID, slot));
		itemFrame.BattlePassButton:SetShown(not itemFrame.ClaimQuest:IsShown() and not sortedSources[i].collected and battlePassSource);
		itemFrame.StoreButton:SetShown(not itemFrame.ClaimQuest:IsShown() and not itemFrame.BattlePassButton:IsShown() and not sortedSources[i].collected and storeSource and not subscriptionSource);
		itemFrame.SubscriptionButton:SetShown(not itemFrame.ClaimQuest:IsShown() and not itemFrame.BattlePassButton:IsShown() and not sortedSources[i].collected and not storeSource and subscriptionSource);
		itemFrame.StoreSubscriptionButton:SetShown(not itemFrame.ClaimQuest:IsShown() and not itemFrame.BattlePassButton:IsShown() and not sortedSources[i].collected and storeSource and subscriptionSource);
	end
...
function WardrobeSetsCollectionMixin:RefreshCameras()
	if ( self:IsShown() ) then
		...
		--if ( model.cameraID ~= detailsCameraID ) then
			...
			-- 修复：3.3.5 GetUICameraInfo 可能返回 nil(高版本相机数据缺失)，给默认值防算术崩溃
			model.defaultPosX = model.defaultPosX or 0;
			model.defaultPosY = model.defaultPosY or 0;
			model.defaultPosZ = model.defaultPosZ or 0;
			model.defaultRotation = model.yaw or 0;
			model.defaultZoom = model.defaultPosX / (model.portraitZoomMultiplier or 3.0);
