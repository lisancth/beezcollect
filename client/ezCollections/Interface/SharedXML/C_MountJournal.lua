...
local function PrepareFilter()
    ...
    _showAccount = C_MountJournal.GetCollectedFilterSetting(LE_MOUNT_JOURNAL_FILTER_ACCOUNT);
...
local function MatchesFilter(mountID)
    ...
    elseif ezCollections:IsActiveMountAccountMount(mountID) then
        if not _showAccount then
            return false;
        end
    ...
    if not _showUnusable then
        ...
        if usableFunc and not isCollected then
            if _G.type(usableFunc) == "function" and not usableFunc() then
                return false;
            elseif _G.type(usableFunc) == "string" and not ezCollections:TestRequirementsString(usableFunc) then
                return false;
            end
        end
    end
...
local function AutoUnshift()
    if ezCollections.Features.AllowMountsAutoUnshift and ezCollections.Config.Wardrobe.MountsAutoUnshift then
        if not InCombatLockdown() and GetShapeshiftForm() > 0 and GetCVarBool("autoUnshift") and (not IsFlying() or GetCVarBool("autoDismountFlying")) then
            ezCollections:SendAddonMessage("MOUNT:UNSHIFT");
        end
    end
end
...
function C_MountJournal.RefreshMounts() -- Custom
    ...
    for mountID, info in pairs(ezCollections.Mounts) do
        local _, _, _, name = unpack(info);
        if name then
            table.insert(_mountIDs, mountID);
        end
    end
    for mountID, info in pairs(ezCollections.Cache.Mounts) do
        if type(mountID) == "number" then
            local _, _, _, name = unpack(info);
            if name and not tContains(_mountIDs, mountID) then
                table.insert(_mountIDs, mountID);
            end
        end
    end
...
function C_MountJournal.GetMountInfoByID(mountID)
    ...
    if companionIndex then
        ...
        -- Exclude from Journal if missing DB2 data
        if not flags then
            shouldHideOnChar = true;
        end
    end
