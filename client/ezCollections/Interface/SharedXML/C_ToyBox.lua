...
local ignorableRedTooltipTexts =
{
    ["Axe"] = true,          ["Топор"] = true,
    ["Bow"] = true,          ["Лук"] = true,
    ["Gun"] = true,          ["Огнестрельное"] = true,
    ["Mace"] = true,         ["Дробящее"] = true,
    ["Polearm"] = true,      ["Древковое"] = true,
    ["Sword"] = true,        ["Меч"] = true,
    ["Staff"] = true,        ["Посох"] = true,
    ["Exotic"] = true,       ["Экзотическое"] = true,
    ["Fist Weapon"] = true,  ["Кистевое"] = true,
    ["Dagger"] = true,       ["Кинжал"] = true,
    ["Thrown"] = true,       ["Метательное"] = true,
    ["Spear"] = true,        ["Копье"] = true,
    ["Crossbow"] = true,     ["Арбалет"] = true,
    ["Wand"] = true,         ["Жезл"] = true,
    ["Fishing Pole"] = true, ["Удочка"] = true,

    ["Cloth"] = true,        ["Ткань"] = true,
    ["Leather"] = true,      ["Кожа"] = true,
    ["Mail"] = true,         ["Кольчуга"] = true,
    ["Plate"] = true,        ["Латы"] = true,
    ["Shield"] = true,       ["Щит"] = true,
    ["Libram"] = true,       ["Манускрипт"] = true,
    ["Idol"] = true,         ["Идол"] = true,
    ["Totem"] = true,        ["Тотем"] = true,
    ["Sigil"] = true,        ["Печати"] = true,
}
local function IsIgnorableRedTooltipText(text)
    if ignorableRedTooltipTexts[text] then
        return true;
    end
    for _, tag in ipairs(ezCollections.InvTypeEnumToName) do
        if text == _G[tag] then
            return true;
        end
    end
...
local function IsToyUsable(itemID)
    ...
    if GetItemInfo(itemID) then
        ...
        local result = tooltip:IsShown() and not ezCollections:ForEachTooltipText(tooltip, function(line)
            ...
            return text and text ~= "" and text ~= " " and ezCollections.IsSameColor(1, 0.125, 0.125, line:GetTextColor()) and not IsIgnorableRedTooltipText(text);
        end);
        tooltip:Hide();
        return result;
    end
