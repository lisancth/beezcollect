...
local function PrepareFilter()
    ...
    _showAccount = C_PetJournal.IsFilterChecked(LE_PET_JOURNAL_FILTER_ACCOUNT);
...
local function MatchesFilter(petID)
    ...
    elseif ezCollections:IsActivePetAccountPet(petID) then
...
function C_PetJournal.RefreshPets() -- Custom
    ...
    -- 修复：宠物表 name 字段全为 nil(靠运行时 GetSpellInfo 动态取名)，原 `if name` 过滤把所有176个宠物都滤掉了，
    -- 导致列表空白、界面用空槽位的马图标填满。改用 creatureID(第1字段)判断：有 creatureID 即为有效宠物。
    for petID, info in pairs(ezCollections.Pets) do
        local creatureID = unpack(info);
        if creatureID then
            table.insert(_petIDs, petID);
        end
    end
    for petID, info in pairs(ezCollections.Cache.Pets) do
        if type(petID) == "number" then
            local creatureID = unpack(info);
            if creatureID and not tContains(_petIDs, petID) then
                table.insert(_petIDs, petID);
            end
        end
    end
...
function C_PetJournal.GetPetInfoBySpeciesID(speciesID)
    ...
    if companionIndex then
        ...
        -- 修复：宠物表第1字段(creatureID)是【生物ID】不是法术ID。名字已直接写入本地表 name 字段(从creature_template的中文名)。
        -- 图标：creatureID 当法术取会得到无关技能图标(攻击/旋风斩的图标)，不可靠；统一用宠物笼图标占位。
        name = name or GetSpellInfo(creatureID) or ("宠物 #"..tostring(petID));
