...
function C_TransmogSets.GetSetHyperlink(transmogSetID, undecorated)
    ...
    return format("|cffff80ff|Hitem:0:transmogset:%d|h[%s]|h|r", transmogSetID, undecorated and name or format(ezCollections.L["Link.Set"], name));
end
...
function C_TransmogSets.GetSourcesForSlot(transmogSetID, slot)
    ...
    if set then
        ...
        for _, source in ipairs(set.sources) do
            ...
            if info and info.InventoryType and C_Transmog.GetSlotForInventoryType(info.InventoryType) == slot then
                local isCollected = ezCollections:HasAvailableSkin(source.id);
                local useError = not ezCollections:CanTransmogrify(source.id) and "" or nil;
                table.insert(sources,
                {
                    ...
                    quality = quality,
                    sourceID = source.id,
                    visualID = ezCollections:GetGroupedVisualID(source.id, info),
                    useError = useError,
