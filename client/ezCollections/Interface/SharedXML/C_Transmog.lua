...
function C_Transmog.GetSlotVisualInfo(slot, transmogType)
    ...
    local baseVisualID = ezCollections:GetGroupedVisualID(baseSourceID);
    local appliedSourceID = fake;
    local appliedVisualID = ezCollections:GetGroupedVisualID(appliedSourceID);
    local pendingSourceID = _pending[slot] and _pending[slot][transmogType] or 0;
    local pendingVisualID = ezCollections:GetGroupedVisualID(pendingSourceID);
