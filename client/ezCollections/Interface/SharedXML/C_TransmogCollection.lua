...
local function GetBossFilterCVar()
    return WardrobeFrame_IsAtTransmogrifier() and "transmogrifyBossFilters" or "wardrobeBossFilters";
end
...
local function GetGroupCVar()
    return WardrobeFrame_IsAtTransmogrifier() and "transmogrifyGroup" or "wardrobeGroup";
end

local function GetSortCVar()
    return WardrobeFrame_IsAtTransmogrifier() and "transmogrifySort" or "wardrobeSort";
end

local function GetSortFavoritesCVar()
    return WardrobeFrame_IsAtTransmogrifier() and "transmogrifySortFavorites" or "wardrobeSortFavorites";
end

local function GetSortCollectedCVar()
    return WardrobeFrame_IsAtTransmogrifier() and "transmogrifySortCollected" or "wardrobeSortCollected";
end
...
local function PrepareFilter()
    ...
    _hideBosses = ezCollections:GetCVar(GetBossFilterCVar());
end

local function GetAppearanceUiOrder(sort, index, numResults, id, info)
    if sort == 0 then
        -- index is included to maintain alphabetic sorting within the same visual group
        -- divided by numResults instead of e.g. 100000 to minimize the chance of floating point precision errors
        return (info.UiOrderByVisual or 0) - index / (numResults + 1);
    elseif sort == 1 then
        return index;
    elseif sort == -1 then
        return -index;
...
local function IsAppearanceCollected(id, info, visual)
    if visual then
        return ezCollections:HasAvailableVisual(id, info) or false;
    else
        return ezCollections:HasAvailableSkin(id) or false;
    end
end

local function MatchesFilter(id)
    local info = ezCollections:GetSkinInfo(id);
    local isCollected = IsAppearanceCollected(id, info, C_TransmogCollection.GetGroup());
    ...
    if _armorTypeMask ~= _fullMask and not (info.InventoryType == 5 or info.InventoryType == 17 or info.InventoryType == 20) then
    ...
    if _hideBosses and next(_hideBosses) and info.SourceBosses then
...
local function GetDBOrCachedSearchResults(category)
    ...
    if not search and not WardrobeFrame_IsAtTransmogrifier() then
        local db = ezCollections:GetDBByCategory(category, false);
        if db.Loaded then
            return db;
        end
        return;
...
function C_TransmogCollection.GetCategoryAppearances(category)
    ...
    local insertedVisuals = { };
    local group = C_TransmogCollection.GetGroup();
    local sort = C_TransmogCollection.GetSort();
    local numResults = #results;
    for index, id in ipairs(results) do
        if type(id) == "number" then
            local matchesFilter, isCollected, isUsable, hasActiveRequiredHoliday = MatchesFilter(id);
            local info = matchesFilter and ezCollections:GetSkinInfo(id);
            if group and info and info.VisualID then
                if not insertedVisuals[info.VisualID] then
                    insertedVisuals[info.VisualID] = true;
                else
                    info = nil;
                end
            end
            if info then
                table.insert(visualsList,
                {
                    visualID = ezCollections:GetGroupedVisualID(id, info),
                    sourceID = id,
                    ...
                    uiOrder = GetAppearanceUiOrder(sort, index, numResults, id, info),
                    -- Custom
                    isClaimable = ezCollections:AnyVisualSources(id, function(sourceID) return ezCollections:CanClaimSkin(sourceID) end),
                    isStoreSource = ezCollections:AnyVisualSources(id, function(sourceID) return ezCollections:IsStoreItem(sourceID) end),
                    isSubscriptionSource = ezCollections:AnyVisualSources(id, function(sourceID) return ezCollections:GetSubscriptionForSkin(sourceID) end),
                    isBattlePassSource = ezCollections:AnyVisualSources(id, function(sourceID) return ezCollections:IsBattlePassItem(sourceID) end),
                });
            end
        end
    end
...
function C_TransmogCollection.GetCategoryTotal(category, visual)
    local db = ezCollections:GetDBByCategory(category, visual);
    if db.Loaded then
        return #db - (C_TransmogCollection.IsSourceTypeFilterChecked(TRANSMOG_SOURCE_STORE) and db.StoreExclusiveCount or 0)
                    ...
                    - (C_TransmogCollection.IsSourceTypeFilterChecked(TRANSMOG_SOURCE_BATTLE_PASS) and db.BattlePassExclusiveCount or 0);
    end
    return 0;
end

function C_TransmogCollection.GetCategoryCollectedCount(category, visual)
    ...
    local hideBattlePass = C_TransmogCollection.IsSourceTypeFilterChecked(TRANSMOG_SOURCE_BATTLE_PASS);
    local collected = 0;
    local db = ezCollections:GetDBByCategory(category, visual);
    if db.Loaded then
        for index, id in ipairs(db) do
            if type(id) == "number" then
                local info = ezCollections:GetSkinInfo(id);
                local hide = hideStore and ezCollections:IsStoreExclusiveItem(id, info, visual)
                            or hideSubscription and ezCollections:IsSubscriptionExclusiveItem(id, info, visual)
                            or hideStore and hideSubscription and ezCollections:IsStoreAndSubscriptionExclusiveItem(id, info, visual)
                            or hideBattlePass and ezCollections:IsBattlePassExclusiveItem(id, info, visual);
                if not hide and IsAppearanceCollected(id, info, visual) then
...
function C_TransmogCollection.GetAppearanceSourceInfo(appearanceSourceID)
    ...
    local appearanceVisualID = ezCollections:GetGroupedVisualID(appearanceSourceID, info);
...
function C_TransmogCollection.GetAppearanceInfoBySource(appearanceSourceID)
    ...
    local result =
    {
        appearanceID = ezCollections:GetGroupedVisualID(appearanceSourceID, info),
        appearanceIsCollected = false,
        sourceIsKnown = true,
        sourceIsCollected = ezCollections:HasAvailableSkin(appearanceSourceID) or C_TransmogCollection.IsAppearanceHiddenVisual(appearanceSourceID),
        sourceIsCollectedPermanent = true,
    };
    --result.sourceIsCollectedPermanent = result.sourceIsCollected; -- TODO: Items in inventory should appear as sources too
    if C_TransmogCollection.GetGroup() then
        result.appearanceIsCollected = IsAppearanceCollected(appearanceSourceID, info, C_TransmogCollection.GetGroup());
    else
        result.appearanceIsCollected = result.sourceIsCollected;
    end
    if C_TransmogCollection.IsAppearanceHiddenVisual(appearanceSourceID) then
        result.appearanceIsCollected = true;
        result.sourceIsKnown = true;
        result.sourceIsCollected = true;
        result.sourceIsCollectedPermanent = true;
    end
    return result;
end
...
function C_TransmogCollection.GetAppearanceCameraID(visualID, fallbackCategory)
    if type(visualID) == "number" then
        ...
        if visualID ~= ezCollections:GetHiddenVisualItem() then
            ...
            if not invType then
                invType = info and ezCollections.InvTypeEnumToName[(info.InventoryType or 0) - 1];
            end
            if not invType then
...
function C_TransmogCollection.GetIllusionSourceInfo(sourceID)
    ...
    local visualID = ezCollections:GetGroupedVisualID(sourceID);
    return visualID, sourceID and ezCollections:TransformEnchantName(select(2, GetItemInfo(sourceID)) or "");
end
...
function C_TransmogCollection.GetAppearanceSources(appearanceID, category)
    ...
    local results;
    if category and WardrobeFrame_IsAtTransmogrifier() and not C_TransmogCollection.IsAppearanceHiddenVisual(appearanceID) then
        results = GetDBOrCachedSearchResults(category);
    end

    for _, sourceID in ezCollections:GetGroupedVisualSources(appearanceID) do
        local name, _, quality = GetItemInfo(sourceID);
        if quality ~= nil and quality > 6 then quality = 6; end
        local isCollected = ezCollections:HasAvailableSkin(sourceID);
        local useError = not (ezCollections:CanTransmogrify(sourceID) and (not results or tContains(results, sourceID))) and "" or nil;
        local info = ezCollections:GetSkinInfo(sourceID);
        table.insert(sources,
        {
            isCollected = isCollected,
            sourceIsCollectedPermanent = isCollected, -- TODO: Items in inventory should appear as sources too
            name = C_TransmogCollection.IsAppearanceHiddenVisual(sourceID) and ezCollections:GetHiddenVisualItemName(slot) or name,
            quality = quality,
            sourceID = sourceID,
            visualID = ezCollections:GetGroupedVisualID(appearanceID, info),
            useError = useError,
            useErrorType = useError and 0 or nil,
            sourceType = MakeSourceType(info),
            -- Custom
            sourceText = MakeSourceText(info),
        });
...
function C_TransmogCollection.GetIsAppearanceFavorite(appearanceID)
    -- Should really only deal with visualID, not sourceID, but for legacy reasons we'll support both
    local container = ezCollections:GetFavoritesContainer();
    for _, sourceID in ezCollections:GetGroupedVisualSources(appearanceID) do
        if container[sourceID] then
            return true;
        end
    end
    return false;
...
function C_TransmogCollection.SetIsAppearanceFavorite(visualID, set)
    -- Should really only deal with visualID, not sourceID, but for legacy reasons we'll support both
    local container = ezCollections:GetFavoritesContainer();
    for _, sourceID in ezCollections:GetGroupedVisualSources(visualID) do
        container[sourceID] = set or nil;
    end
...
function C_TransmogCollection.IsNewAppearance(visualID)
    for _, sourceID in ezCollections:GetGroupedVisualSources(visualID) do
        if ezCollections.Config.TransmogCollection.NewAppearances[sourceID] then
            return true;
        end
    end
end
...
function C_TransmogCollection.ClearNewAppearance(visualID)
    for _, sourceID in ezCollections:GetGroupedVisualSources(visualID) do
        ezCollections.Config.TransmogCollection.NewAppearances[sourceID] = nil;
    end
...
function C_TransmogCollection.AddNewAppearance(sourceID) -- Custom
    ezCollections.Config.TransmogCollection.NewAppearances[sourceID] = true;

    local skin = ezCollections:GetSkinInfo(sourceID);
    local visualID = ezCollections:GetGroupedVisualID(sourceID, skin);
    ...
    if skin and skin.Sets then
        ezCollections.Config.TransmogCollection.NewSetSources[sourceID] = true;
        ezCollections.Config.TransmogCollection.LatestSetSource = sourceID;
    end
end
...
function C_TransmogCollection.SetGroup(value)
    if ezCollections:SetCVar(GetGroupCVar(), value) then
        SearchUpdated();
    end
end

function C_TransmogCollection.SetSort(value)
    if ezCollections:SetCVar(GetSortCVar(), value) then
        SearchUpdated();
    end
end

function C_TransmogCollection.SetSortFavorites(value)
    if ezCollections:SetCVar(GetSortFavoritesCVar(), value) then
        SearchUpdated();
    end
end

function C_TransmogCollection.SetSortCollected(value)
    if ezCollections:SetCVar(GetSortCollectedCVar(), value) then
        SearchUpdated();
    end
end
...
function C_TransmogCollection.GetGroup()
    return ezCollections:GetCVar(GetGroupCVar());
end

function C_TransmogCollection.GetSort()
    return ezCollections:GetCVar(GetSortCVar());
end

function C_TransmogCollection.GetSortFavorites()
    return ezCollections:GetCVar(GetSortFavoritesCVar());
end

function C_TransmogCollection.GetSortCollected()
    return ezCollections:GetCVar(GetSortCollectedCVar());
end
...
function C_TransmogCollection.SetBossFilter(i, value) -- Custom
    ezCollections:GetCVar(GetBossFilterCVar())[i] = not value or nil;
    SearchUpdated();
end

function C_TransmogCollection.IsBossFilterChecked(i) -- Custom
    return ezCollections:GetCVar(GetBossFilterCVar())[i];
end

function C_TransmogCollection.SetBossFilterByInstance(i, value) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for id in pairs(ezCollections.Encounters) do
        local encounter = ezCollections:GetEncounterInfo(id);
        if encounter.Map == i then
            filter[id] = not value or nil;
        end
    end
    SearchUpdated();
end

function C_TransmogCollection.IsBossFilterCheckedByInstance(i) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for id in pairs(ezCollections.Encounters) do
        local encounter = ezCollections:GetEncounterInfo(id);
        if encounter.Map == i then
...
function C_TransmogCollection.SetBossFilterByInstanceDifficulty(map, difficulty, value) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for id in pairs(ezCollections.Encounters) do
        local encounter = ezCollections:GetEncounterInfo(id);
        if encounter.Map == map and encounter.DifficultyIndex == difficulty then
            filter[id] = not value or nil;
        end
    end
    SearchUpdated();
...
function C_TransmogCollection.IsBossFilterCheckedByInstanceDifficulty(map, difficulty) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for id in pairs(ezCollections.Encounters) do
        local encounter = ezCollections:GetEncounterInfo(id);
        if encounter.Map == map and encounter.DifficultyIndex == difficulty then
...
function C_TransmogCollection.SetBossFilterByInstanceTier(i, value) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for map in pairs(ezCollections.Instances) do
        local instance = ezCollections:GetInstanceInfo(map);
        if instance.Tier == i then
            for id in pairs(ezCollections.Encounters) do
                local encounter = ezCollections:GetEncounterInfo(id);
                if encounter.Map == map then
                    filter[id] = not value or nil;
                end
            end
        end
...
function C_TransmogCollection.IsBossFilterCheckedByInstanceTier(i) -- Custom
    local filter = ezCollections:GetCVar(GetBossFilterCVar());
    for map in pairs(ezCollections.Instances) do
        local instance = ezCollections:GetInstanceInfo(map);
        if instance.Tier == i then
...
function C_TransmogCollection.SetDefaultFilters()
    ...
    ezCollections:SetCVar(GetBossFilterCVar(), { });
...
function C_TransmogCollection.IsUsingDefaultFilters()
    return ezCollections:GetCVar(GetSourceFilterCVar()) == 0
        ...
        and not next(ezCollections:GetCVar(GetBossFilterCVar()))
