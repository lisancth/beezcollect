...
function ezCollections:GetCharacterCameraID(invTypeName, cameraID)
    ...
    -- 修复：魔改种族(狼人/熊猫人等)不在 RaceToCameraID 表中返回 nil 导致算术崩溃，所有查表结果全部兜底
    local race = self.RaceToCameraID[select(2, UnitRace("player"))] or self.RaceToCameraID["Human"] or 0;
    local sex = self.SexToCameraID[UnitSex("player")] or 0;
    fallback = fallback or 0;
    option = option or fallback;
    if cameraID then
    ...
    local invType = self.InvTypeToCameraID[invTypeName] or 0;
