...
- **采集箱(GO)关键坑**: type=10 GOOBER补给箱,配置照狼人箱60101(Data1=questId, Data5=1)。
  ★重要: 新建/改动GameObject后【必须重启worldserver】才能完全生效(GO交互判定在加载时建立,
  热改不够)。重启后接任务→箱子发光→鼠标有齿轮→点击完成→消失30秒刷新。全部正常。
